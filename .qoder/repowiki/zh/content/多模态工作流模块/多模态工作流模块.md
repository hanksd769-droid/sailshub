...
- **音频处理：支持从 URL 下载音频并上传至 Coze，提供批量音频处理能力**
...
BE_R_Audio["routes/audio.ts<br/>音频处理(URL上传)"]
...
- [api/src/routes/audio.ts:1-161](file://api/src/routes/audio.ts#L1-L161)
...
- [web/src/lib/api.ts:1-239](file://web/src/lib/api.ts#L1-L239)
...
- [web/src/pages/MixCutPage.tsx:1-355](file://web/src/pages/MixCutPage.tsx#L1-L355)
...
- **音频处理服务：支持从 URL 下载音频并上传至 Coze，提供批量音频处理能力**
...
- [api/src/routes/audio.ts:14-158](file://api/src/routes/audio.ts#L14-L158)
- [web/src/lib/api.ts:210-239](file://web/src/lib/api.ts#L210-L239)
...
FE->>API : POST /api/audio/upload-from-url
API->>API : 下载音频文件
...
**图表来源**
- [api/src/routes/runs.ts:55-159](file://api/src/routes/runs.ts#L55-L159)
- [api/src/routes/audio.ts:14-81](file://api/src/routes/audio.ts#L14-L81)
...
### 音频处理服务
**新增** 音频处理模块提供了从 URL 下载音频并上传至 Coze 的完整能力，支持单个和批量音频处理。

#### 单个音频上传
- 接口：POST /api/audio/upload-from-url
- 参数：{ url: string }
- 行为：下载音频文件 → 上传至 Coze → 返回 file_id 和 file_url
- 返回：{ success, data: { file_id, file_url, coze_response } }

#### 批量音频上传
- 接口：POST /api/audio/batch-upload-from-urls
- 参数：{ urls: string[] }
- 行为：循环处理每个 URL → 下载 → 上传 → 记录结果和错误
...
A["输入音频 URL"] --> B["下载音频文件"]
B --> C["构造 FormData"]
C --> D["POST /v1/files/upload 到 Coze"]
D --> E{"上传成功?"}
E --> |是| F["返回 file_id 和 file_url"]
E --> |否| G["记录错误并继续下一个"]
F --> H["返回成功结果"]
G --> I["返回汇总统计"]
```
...
**图表来源**
- [api/src/routes/audio.ts:14-81](file://api/src/routes/audio.ts#L14-L81)
- [api/src/routes/audio.ts:88-158](file://api/src/routes/audio.ts#L88-L158)
...
MixPage->>API : batchUploadAudioFromUrls()
API->>AudioRoute : POST /api/audio/batch-upload-from-urls
AudioRoute->>Coze : 下载并上传音频
...
**图表来源**
- [web/src/pages/MixCutPage.tsx:66-146](file://web/src/pages/MixCutPage.tsx#L66-L146)
- [web/src/lib/api.ts:225-239](file://web/src/lib/api.ts#L225-L239)
- [api/src/routes/audio.ts:88-158](file://api/src/routes/audio.ts#L88-L158)
...
Voice --> API
Mix --> API
Mix --> AudioAPI["api.ts batchUploadAudioFromUrls"]
```
...
- [api/src/routes/audio.ts:74-80](file://api/src/routes/audio.ts#L74-L80)

## 结论
本多模态工作流模块通过清晰的模块化设计与 SSE 流式传输，实现了从图像生成、视频文案提取、文案创作到翻译、语音合成和音频处理的完整链路。新增的音频处理模块进一步扩展了多模态能力，特别是混剪功能的集成展示了音频处理在实际业务场景中的应用价值。前后端职责明确，后端统一接入 Coze 与本地语音服务，前端提供直观的交互与结果展示。建议在生产环境中加强鉴权、日志与监控，并针对高频路径引入缓存与异步处理以进一步提升性能。
...
### 前端 API 函数
- **uploadAudioFromUrl(url: string)**：单个音频上传
- **batchUploadAudioFromUrls(urls: string[])**：批量音频上传
