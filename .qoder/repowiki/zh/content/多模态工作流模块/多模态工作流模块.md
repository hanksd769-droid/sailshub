...
- 后端 API（TypeScript + Express）
  - 路由层：认证、模块查询、任务运行、文件上传、语音服务
  - 数据层：PostgreSQL 连接与初始化
  - 配置层：环境变量校验与读取
  - 中间件：鉴权
- 前端 Web（React + Vite）
  - 页面：仪表盘、各功能页、结果面板
  - 工具：统一 API 封装、SSE 读取、文件上传
...
## 结论
本多模态工作流模块通过清晰的模块化设计与 SSE 流式传输，实现了从图像生成、视频文案提取、文案创作到翻译与语音合成的完整链路。前后端职责明确，后端统一接入 Coze 与本地语音服务，前端提供直观的交互与结果展示。建议在生产环境中加强鉴权、日志与监控，并针对高频路径引入缓存与异步处理以进一步提升性能。
...
- POST /api/voice/tts-from-lines
  - 请求体：{ lines: string[] }
  - 返回：{ success, data: { lines, txt, tts }, debugId, debugUrl }
