...
## 更新摘要
**变更内容**
- 音频上传机制完全重构：从URL下载上传改为本地文件路径直接上传
- 新增本地文件上传API端点：`/api/audio/upload-local` 和 `/api/audio/batch-upload-local`
- 前端API客户端更新：新增 `uploadAudioFromLocal` 和 `batchUploadAudioFromLocal` 函数
- URL路径解析增强：支持从URL中提取本地文件路径参数
...
**图表来源**
- [audio.ts:1-182](file://api/src/routes/audio.ts#L1-L182)
...
**更新** 本次更新反映了音频上传机制的重大重构，从URL下载上传完全转变为本地文件路径直接上传，显著提升了处理效率和可靠性。