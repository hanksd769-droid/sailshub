...
## 更新摘要
**变更内容**
- **增强翻译兼容性**：extractOutputArray函数支持更多输出字段变体（'translated'、'translation'、'lines'、'items'），提升与不同翻译服务的兼容性
- **新增多模式处理能力**：语音生成接口支持逐条配音、合并配音和双模式输出三种模式
- **移除文件上传依赖**：语音生成现在直接使用文本输入，简化了 TTS 流程
- **增强 TTS 生成能力**：支持灵活的模式选择和混合输出
- **优化前端集成**：ProductCopyPage 等页面支持多模式语音生成工作流
- **简化文件上传接口**：后端直接处理上传的文件，不再需要复杂的临时文件处理逻辑
...
DI["DetailImagePage.tsx"]
VC["VideoCopyPage.tsx"]
API["lib/api.ts"]
end
subgraph "后端 API"
IDX["index.ts"]
...
- [web/src/pages/ProductCopyPage.tsx:1-456](file://web/src/pages/ProductCopyPage.tsx#L1-L456)
...
## 核心组件
- 语音路由模块：提供 /api/voice 下的配置查询、批量翻译、多模式 TTS 生成等接口。
...
- [web/src/lib/api.ts:117-163](file://web/src/lib/api.ts#L117-L163)

## 架构总览
语音生成的整体流程如下：
- 前端调用 /api/voice/config 获取语音服务地址
- 可选：调用 /api/voice/translate-lines 对文案进行批量翻译，得到英文行数组
- 调用 /api/voice/tts-from-lines 传入英文行数组和模式参数，触发多模式 TTS 流程
- 后端通过 Gradio 客户端连接局域网语音 Studio，执行生成任务
- 支持逐条配音、合并配音和双模式输出三种模式
...
FE->>API : POST /api/voice/tts-from-lines {lines, mode}
API->>GR : connect(base) 并根据模式执行 TTS
GR-->>API : 返回音频与字幕等结果
API-->>FE : 返回多模式 TTS 结果含调试 ID
```

**图表来源**
- [api/src/routes/voice.ts:66-83](file://api/src/routes/voice.ts#L66-L83)
- [api/src/routes/voice.ts:282-329](file://api/src/routes/voice.ts#L282-L329)
- [api/src/routes/voice.ts:331-419](file://api/src/routes/voice.ts#L331-L419)
- [api/src/routes/files.ts:10-40](file://api/src/routes/files.ts#L10-L40)
- [api/src/coze.ts:4-7](file://api/src/coze.ts#L4-L7)
...
**更新** 简化了文件上传流程，直接处理前端传入的 File 对象
...
- 权限：需要认证
- 请求体：lines（英文字符串数组）、mode（'individual' | 'merged' | 'both'）
- 处理逻辑：
  - 连接 Gradio Studio（通过配置的语音服务地址）
  - 支持三种模式：
    - `individual`：逐条生成，对每一行单独执行 TTS
    - `merged`：整段生成，将所有行合并后执行 TTS
    - `both`：双模式输出，同时生成两种结果
  - 配置多种语音参数（音色、语速、情感表达等）
  - 触发生成音频，返回多模式结果
- 返回：lines、mode、results（包含 individual 和/或 merged）
- 调试：记录每个步骤的输入与输出

**更新** 新增多模式处理能力，支持灵活的 TTS 生成策略
...
- **更新**：文件上传：uploadFile（直接使用原生 File 对象进行上传）
- 语音页面：VoiceGeneratorPage.tsx 展示 studioUrl 与 apiUrl，并在受限制情况下提示使用新标签页打开
- 产品文案页面：ProductCopyPage.tsx 实现完整的三步工作流（生成文案 → 独立英译 → 生成语音），支持多模式输出
- **更新**：详情图片页面：DetailImagePage.tsx 直接使用 File 对象进行文件上传
- **更新**：视频复制页面：VideoCopyPage.tsx 直接使用 File 对象进行文件上传
...
- [web/src/pages/DetailImagePage.tsx:115-154](file://web/src/pages/DetailImagePage.tsx#L115-L154)
- [web/src/pages/VideoCopyPage.tsx:71-91](file://web/src/pages/VideoCopyPage.tsx#L71-L91)
...
- **更新**：TTS 处理流程简化，移除了临时文件创建和文件上传步骤，减少了 I/O 操作
- **更新**：多模式处理支持灵活的资源分配，可根据需求选择最优模式
...
- **更新**：文件上传使用原生 File 对象，支持大文件传输但需要注意网络带宽限制
...
- **更新**：TTS 生成失败
  ...
  - 参考：[api/src/routes/voice.ts:374-419](file://api/src/routes/voice.ts#L374-L419)
- **更新**：多模式 TTS 参数错误
  - 现象：传入无效的 mode 参数
  - 排查：确认 mode 参数为 'individual'、'merged' 或 'both'
  - 参考：[api/src/routes/voice.ts:331-352](file://api/src/routes/voice.ts#L331-L352)
- **更新**：文件上传失败
...
## 结论
本语音生成接口通过后端路由聚合翻译与 TTS 能力，借助 Coze 工作流与 Gradio Studio 实现高质量语音合成与字幕导出。**更新**后的多模式 TTS 处理管道支持逐条配音、合并配音和双模式输出，提供了更灵活的语音生成策略。移除了临时文件创建和文件上传机制，简化了流程并提升了性能。前端通过原生 File 对象直接进行文件上传，增强了兼容性和用户体验。**新增的增强翻译兼容性功能**使得系统能够更好地适配不同翻译服务的输出格式，通过支持 'translated'、'translation'、'lines'、'items' 等多种字段变体，显著提升了系统的鲁棒性和适用范围。建议在生产环境中完善调试持久化、错误监控与网络优化，确保稳定与低延迟的服务体验。
...
Client->>VoiceAPI : POST /api/voice/tts-from-lines {lines, mode}
VoiceAPI->>Studio : connect(base) 并根据模式执行 TTS
Studio-->>VoiceAPI : 返回音频与字幕
VoiceAPI-->>Client : 返回多模式结果含调试 ID
```
...
- **更新**：多模式选择策略
  - 逐条配音：适合需要精确控制每行语音的场景
  - 合并配音：适合整体语音合成，减少生成次数
  - 双模式输出：同时获得两种结果，便于对比和选择
- **更新**：错误恢复
  - 使用调试接口快速定位问题；TTS 阶段检查 Gradio Studio 连接状态
  - 文件上传阶段检查 Coze API Token 和网络连接
- 性能优化
  - 局域网部署语音服务；合理设置请求体大小与超时时间；缓存常用配置
  - **更新**：优化文件上传流程，直接使用原生 File 对象减少中间步骤
  - **更新**：多模式处理支持按需选择，避免不必要的资源消耗
...
#### 增强翻译兼容性
- **功能**：extractOutputArray函数支持更多输出字段变体（'translated'、'translation'、'lines'、'items'）
- **实现**：在对象字段提取逻辑中增加对多种字段名的支持，提升与不同翻译服务的兼容性
- **优势**：能够适配更多第三方翻译服务的输出格式，减少因字段命名差异导致的解析失败
- **应用场景**：支持来自不同翻译平台的数据格式，提高系统的通用性和稳定性

#### 多模式 TTS 生成能力
- **功能**：支持逐条配音、合并配音和双模式输出三种模式
- **实现**：根据 mode 参数动态选择处理策略
- **逐条模式**：对每一行单独执行 TTS，适合需要精确控制的场景
- **合并模式**：将所有行合并后执行 TTS，适合整体语音合成
- **双模式输出**：同时生成两种结果，便于对比和选择
- **优势**：提供灵活的语音生成策略，满足不同业务需求

#### 移除文件上传依赖
- **功能**：语音生成现在直接使用文本输入
- **实现**：简化了 TTS 处理步骤，移除了临时文件创建
- **优势**：减少了 I/O 操作，提升了处理速度和稳定性
- **兼容性**：保持了原有的语音参数配置和输出格式

#### 增强的前端文件处理
- **功能**：DetailImagePage 和 VideoCopyPage 直接使用 File 对象进行文件上传
- **实现**：前端直接构造 FormData，包含原生 File 对象
- **优势**：提高了文件处理的原生性和兼容性
- **应用场景**：详情图片上传、视频文件上传等
...
#### 产品文案多模式工作流
- **功能**：ProductCopyPage 支持多模式语音生成工作流
- **实现**：支持 'both' 模式，同时生成逐条和合并结果
- **优势**：提供完整的语音生成解决方案，提升用户体验
- **应用场景**：产品文案生成到语音输出的完整流程

**章节来源**
- [api/src/routes/voice.ts:130-179](file://api/src/routes/voice.ts#L130-L179)
- [api/src/routes/voice.ts:331-419](file://api/src/routes/voice.ts#L331-L419)
- [api/src/routes/voice.ts:208-242](file://api/src/routes/voice.ts#L208-L242)
- [api/src/routes/voice.ts:9-50](file://api/src/routes/voice.ts#L9-L50)
