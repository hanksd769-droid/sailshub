...
## 更新摘要
**所做更改**
- 更新了产品文案生成V2工作流的模块键和名称显示
- 修正了字符编码问题导致的工作流名称乱码显示
- 完善了产品文案生成V2的API参数说明和使用示例
...
## 核心组件
- 模块定义与路由
  - 模块键与工作流 ID 映射：详情图（有/无参考图）、视频提取文案、产品文案生成、产品文案生成V2、翻译功能
...
**章节来源**
- [api/src/modules.ts:1-34](file://api/src/modules.ts#L1-L34)
...
OnMsg --> |Done| MarkSuccess["标记 SUCCESS 并持久化输出"]
OnMsg --> |Error| MarkFailed["标记 FAILED 并持久化错误"]
Push --> Stream
MarkSuccess --> Done(["结束"])
MarkFailed --> Done
```
...
## 结论
该工作流模块接口围绕多模态能力（图像、视频、文案、翻译、语音）构建，通过统一的模块键与工作流 ID 映射、SSE 流式执行与数据库持久化，实现了从请求到结果的全链路可观测性。前端通过标准化 API 客户端与页面组件，提供了良好的用户体验。**更新** 最新修复解决了产品文案生成V2工作流的字符编码问题，确保了工作流名称的正确显示和路由识别。建议后续在监控埋点、限流降级与缓存策略方面进一步完善。
...
- **更新** 产品文案生成V2（带音频）
  ...
  - 参数：
    ...
    - changping：产品名称
    - donzuojiexi：动作解析数组
  - 场景：导入上一步生成的文案数据，调用带音频的工作流生成完整结果
