...
## 简介
本文件为工作流模块接口的详细 API 文档，覆盖多模态工作流相关接口，包括图像生成（详情图）、视频处理（视频提取文案）、文案创作（产品文案生成）、翻译服务等模块。文档重点说明：
...
## 核心组件
- 模块定义与路由
  - 模块键与工作流 ID 映射：详情图（有/无参考图）、视频提取文案、产品文案生成、翻译功能
...
## 结论
该工作流模块接口围绕多模态能力（图像、视频、文案、翻译、语音）构建，通过统一的模块键与工作流 ID 映射、SSE 流式执行与数据库持久化，实现了从请求到结果的全链路可观测性。前端通过标准化 API 客户端与页面组件，提供了良好的用户体验。建议后续在监控埋点、限流降级与缓存策略方面进一步完善。
