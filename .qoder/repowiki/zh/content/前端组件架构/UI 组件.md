...
- **新增**：智能文本格式化功能，支持 JSON 对象和普通文本的差异化展示
...
## 架构总览
ResultPanel 在多个页面中以相同形态出现，但数据来源与状态由各页面管理。页面通过 props 将流式文本、JSON 文本、进度、加载状态、错误信息传递给 ResultPanel；同时提供复制回调函数，实现剪贴板写入。新增的类型系统使组件能够更好地表达不同状态和重要性级别。**最新改进包括智能文本格式化功能，能够自动识别 JSON 对象并进行差异化展示**。
...
Panel->>Panel : 智能文本格式化<br/>JSON对象与普通文本差异化展示
...
- **文本展示区**：**新增** 智能文本格式化功能，支持 JSON 对象和普通文本的差异化展示，深色背景、等宽字体、固定高度、自动滚动，适合日志/流式输出阅读。
...
- ****新增**：智能文本格式化**：自动识别 JSON 对象并进行结构化展示，普通文本按行分割显示，为每个片段添加样式化的边框和内边距，提升可读性。

**更新**：新增类型系统，提供更丰富的状态表达和视觉反馈。**新增智能文本格式化功能**。

**章节来源**
- [ResultPanel.tsx:16-118](file://web/src/components/ResultPanel.tsx#L16-L118)
...
- ****新增**：智能文本格式化性能优化**：使用高效的字符串解析和映射操作，避免重复渲染。
...
- ****新增**：智能文本格式化展示**：自动识别 JSON 对象和普通文本，提供更好的文本可读性和视觉层次。
...
Success --> Format["智能文本格式化<br/>JSON对象结构化展示"]
Primary --> Format
Warning --> Format
...
- ****新增**：智能文本格式化体验优化**：自动识别 JSON 对象和普通文本，提供更好的文本可读性和视觉层次。
...
- ****新增**：文本格式化问题**
  - 确认 streamText 包含有效的 JSON 对象或普通文本。
  - 检查 JSON 解析逻辑是否正常工作。
- **Card 样式问题**
...
## 结论
ResultPanel 已成功从简单 div 布局升级为基于 Ant Design Card 的现代化组件，新增类型系统和增强的视觉层次显著提升了用户体验。通过简洁、受控的方式实现了统一的结果展示，与 Ant Design 生态无缝衔接，适配多页面、多流程的复杂场景。**最新的改进包括智能文本格式化功能和视觉样式优化，能够自动识别 JSON 对象和普通文本并提供更好的文本可读性和视觉层次**。新的类型系统（primary/success/warning）提供了更好的状态表达，Card 结构增强了视觉层次，改进的图标系统提升了视觉一致性。
...
- **新增特性**
  ...
  - **新增**：智能文本格式化：支持 JSON 对象结构化展示和普通文本行分割
...
- [ResultPanel.tsx:68-107](file://web/src/components/ResultPanel.tsx#L68-L107)