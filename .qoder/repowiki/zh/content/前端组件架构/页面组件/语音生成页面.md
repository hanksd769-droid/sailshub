...
**更新** 配置管理系统现在采用更严格的验证机制：

服务配置通过环境变量和动态检测实现，增加了完整的环境变量验证：

- **基础 URL 管理**：统一的语音服务基础地址
- **主题配置**：支持暗色主题的界面配置
- **API 文档链接**：自动生成 API 文档访问地址
- **错误检测**：配置缺失时的及时错误提示
- **环境变量验证**：启动时强制检查必需的环境变量

配置验证机制确保系统在启动时就发现配置问题：
...
**更新** 系统对环境变量有严格的要求，并且现在具备完整的配置验证机制：

- **COZE_API_TOKEN**：Coze API 认证令牌，用于工作流引擎访问
- **DATABASE_URL**：数据库连接字符串，用于数据持久化
- **JWT_SECRET**：JWT 令牌密钥，用于用户认证
- **VOICE_BASE_URL**：语音服务基础地址，用于 Gradio 客户端连接

**配置验证机制**：
- 应用启动时自动检查所有必需环境变量
- 缺少任何必需变量都会导致启动失败并抛出详细错误
- 错误信息明确指出缺少的具体环境变量名称

**前端配置**：
- 前端通过 `VITE_API_BASE` 环境变量配置 API 基础地址
...
- [package.json:11-23](file://api/package.json#L11-L23)
