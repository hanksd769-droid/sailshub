...
- [TTS_FIX_INSTRUCTIONS.md](file://TTS_FIX_INSTRUCTIONS.md)
</cite>
...
- TTS 生成流程：支持从文案结果中提取英文行、批量翻译、构建文本并调用语音生成器进行 TTS（含 MP3 和 SRT 输出），并提供完整的调试与监控。
- 调试与监控：内置调试记录存储、步骤日志与可视化列表，便于定位问题与优化流程。
- 与 Coze AI 的集成：通过 Coze API 工作流执行批量翻译，再由本地语音生成器完成 TTS。
- 前端界面：提供语音生成器页面，展示服务地址与 API 文档链接，并在受保护路由下加载语音生成器 iframe。
- **新增**：完整的 TTS 语音生成功能，包括批量翻译、直接 TTS 生成、调试记录管理等新特性。
...
- 部署：使用 Docker Compose 编排数据库、API 服务与前端静态服务。
...
- [web/src/lib/api.ts:1-162](file://web/src/lib/api.ts#L1-L162)
...
- 语音路由模块：提供语音配置查询、批量翻译、TTS 生成、调试记录管理等接口。
...
- **新增**：反向代理配置：通过 Nginx 实现 API 请求的反向代理，解决局域网访问问题。
...
- 后端提供语音相关接口，内部通过 Coze 工作流进行批量翻译，再连接局域网语音生成器执行 TTS。
- 调试记录在后端内存中维护，支持按 ID 查询与列表查看，便于问题定位。
- **新增**：Nginx 反向代理确保局域网内多设备访问时 API 请求正确路由到后端服务。
...
U->>W : 打开语音生成器页面
W->>N : GET /api/voice/config
N->>A : 转发请求到 API 服务
A->>V : 路由转发
V->>V : 校验 VOICE_BASE_URL
V-->>W : 返回 studioUrl/apiUrl
U->>W : 触发翻译或 TTS 请求
W->>N : POST /api/voice/translate-lines 或 /tts-from-lines
N->>A : 转发请求到 API 服务
A->>V : 路由转发
...
- TTS 生成：将英文行拼接为文本，连接局域网语音生成器，依次设置参数并触发生成，返回音频与字幕结果。
...
BuildTxt --> Connect["连接语音生成器<br/>Client.connect"]
...
- **新增**：动态 API 基础地址配置，支持相对路径和环境变量配置。

**章节来源**
- [web/src/lib/api.ts:1-162](file://web/src/lib/api.ts#L1-L162)

### 应用路由与鉴权（web/src/App.tsx）
- 设置未授权回调，清理本地 Token 并跳转登录页。
- 在应用启动时校验当前会话有效性，异常时自动登出。

**章节来源**
- [web/src/App.tsx:17-39](file://web/src/App.tsx#L17-L39)

### 反向代理配置（web/nginx.conf）
- **新增**：Nginx 反向代理配置，将 `/api/` 路径的请求转发到后端 API 服务。
- 支持 WebSocket 升级，确保实时通信功能正常工作。
- 配置了必要的 HTTP 头部传递，保持客户端信息的完整性。
...
- 语音生成器连接失败
  - 现象：TTS 步骤报连接错误。
  - 处理：确认局域网语音生成器地址可达，检查防火墙与端口配置。
- 调试记录缺失
  - 现象：无法通过 /api/voice/debug/:id 查到记录。
  - 处理：确认调试记录未被清理（超过最大数量会删除最早记录）。
- **新增**：局域网访问问题
  - 现象：其他设备无法访问语音生成器。
  - 处理：检查 Nginx 反向代理配置，确认 API 请求正确转发到后端服务。
- **新增**：API 基础地址配置错误
  - 现象：前端请求指向错误的主机地址。
...
- [TTS_FIX_INSTRUCTIONS.md:1-169](file://TTS_FIX_INSTRUCTIONS.md#L1-L169)
...
- **新增**：完善的局域网部署支持：通过反向代理和动态 API 配置，解决了多设备访问问题。
...
- [web/src/lib/api.ts:2-3](file://web/src/lib/api.ts#L2-L3)

### 集成与部署要点
- Docker Compose：一键启动数据库、API 服务与前端静态服务，端口映射与依赖关系已在编排文件中定义。
- 前端开发：使用 Vite，默认端口 5173；生产构建后由 Nginx 提供静态服务（参考项目中的 nginx.conf）。
- 后端开发：使用 TypeScript 与 TSX，开发模式下热更新。
- **新增**：反向代理配置：Nginx 反向代理确保局域网内多设备访问时 API 请求正确路由到后端服务。
...
**章节来源**
- [TTS_FIX_INSTRUCTIONS.md:65-76](file://TTS_FIX_INSTRUCTIONS.md#L65-L76)
- [api/src/routes/voice.ts:256-273](file://api/src/routes/voice.ts#L256-L273)
- [api/src/routes/voice.ts:276-341](file://api/src/routes/voice.ts#L276-L341)
- [api/src/routes/voice.ts:343-402](file://api/src/routes/voice.ts#L343-L402)