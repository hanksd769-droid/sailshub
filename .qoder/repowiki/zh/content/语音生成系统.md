...
## 更新摘要
**变更内容**
- 临时文件管理系统：新增临时文件管理功能，使用项目目录下的 `temp` 文件夹存储临时 `.txt` 文件
- 增强的错误处理：在 TTS 生成过程中保留临时文件用于调试，便于问题诊断
- 改进的调试功能：增强调试记录系统，提供更详细的步骤跟踪和错误诊断能力
- 临时文件清理机制：通过 `.gitignore` 配置忽略 `temp` 目录，避免意外提交
...
- **新增**：临时文件管理系统，通过项目目录下的 `temp` 文件夹管理临时 `.txt` 文件，支持自动清理和调试保留机制。
- **新增**：增强的错误处理机制，在 TTS 生成失败时保留临时文件便于问题诊断。
...
GITIGNORE[".gitignore<br/>忽略 temp 目录"]
end
WG --> API
API --> IDX
IDX --> VOICE
...
- [api/src/routes/voice.ts:1-411](file://api/src/routes/voice.ts#L1-L411)
...
- 临时文件管理系统：新增的临时文件管理功能，使用项目目录下的 `temp` 文件夹存储临时 `.txt` 文件。
...
- **新增**：增强的错误处理机制：在 TTS 生成失败时保留临时文件，便于问题诊断和调试。
...
- 临时文件通过 `temp` 目录管理，支持自动清理和调试保留机制。
...
- **新增**：临时文件管理：在 `runTtsFromTxt` 函数中实现临时文件管理，使用项目目录下的 `temp` 文件夹存储临时 `.txt` 文件。
...
- **临时文件管理**：新增临时文件管理系统，使用 `temp` 目录存储临时 `.txt` 文件，支持自动清理和调试保留机制。
...
- **新增**：临时文件管理算法：在 `runTtsFromTxt` 函数中实现，包括目录创建、文件写入、错误处理和调试保留机制。
...
BuildTxt --> CreateTemp["创建临时文件<br/>temp/voice-时间戳.txt"]
CreateTemp --> Connect["连接 Gradio 客户端<br/>Client.connect"]
Connect --> SetParams["设置参数<br/>predict(..., {value})"]
SetParams --> Generate["触发生成<br/>predict('/generate_audio')"]
Generate --> Success["返回成功结果"]
Success --> CleanUp["保留临时文件用于调试"]
CleanUp --> Done(["结束"])
Err --> Done
```
...
### 临时文件管理系统
**新增**：完整的临时文件管理功能，包含以下特性：

- **目录管理**：在项目根目录创建 `temp` 文件夹，使用 `process.cwd()` 获取当前工作目录。
- **文件命名**：使用 `voice-时间戳.txt` 格式命名临时文件，确保唯一性。
- **文件写入**：将 TTS 文本内容写入临时文件，使用 UTF-8 编码。
- **错误处理**：在 TTS 生成过程中发生错误时，保留临时文件用于调试分析。
- **调试保留**：在 `runTtsFromTxt` 函数中，即使发生错误也会保留临时文件，便于问题诊断。
- **自动清理**：通过 `.gitignore` 配置忽略 `temp` 目录，避免意外提交到版本控制系统。
...
- **新增**：临时文件管理：临时文件存储在内存中，避免频繁的磁盘 I/O 操作，提高性能。
- **新增**：文件清理策略：通过 `.gitignore` 配置忽略 `temp` 目录，避免意外提交和磁盘空间浪费。
...
- **新增**：临时文件未创建或无法访问
  - 现象：TTS 生成失败但找不到临时文件。
  - 处理：检查项目目录权限，确认 `temp` 文件夹存在且可写；查看调试记录中的 `tts_temp_file_saved` 步骤。
- **新增**：临时文件未保留用于调试
  - 现象：TTS 失败后临时文件被清理。
  - 处理：检查 `runTtsFromTxt` 函数中的错误处理逻辑，确认在 catch 块中保留了临时文件。
- **新增**：API 基础地址配置错误
  - 现象：前端请求指向错误的主机地址。
...
- [api/src/routes/voice.ts:252-253](file://api/src/routes/voice.ts#L252-L253)
...
- **新增**：临时文件管理系统，通过 `temp` 目录管理临时文件，支持自动清理和调试保留机制。
- **新增**：增强的错误处理机制，在 TTS 生成失败时保留临时文件，便于问题诊断和解决。
...
- **新增**：临时文件配置
  - temp 目录：项目根目录下的 `temp` 文件夹，用于存储临时 `.txt` 文件
  - .gitignore：忽略 `temp` 目录，避免意外提交
...
- **新增**：临时文件管理：系统自动管理 `temp` 目录，支持临时文件的创建、使用和清理。
- **新增**：错误处理机制：在 TTS 生成失败时保留临时文件，便于问题诊断和解决。
...
- **调试与监控**：
  ...
  5. **新增**：检查临时文件状态，确认临时文件是否被正确创建和保留
...
- **步骤跟踪内容**：
  ...
  - **新增**：tts_temp_file_saved：临时文件保存信息
  - **新增**：tts_error_keep_file：错误时保留临时文件信息
...
- **临时文件结构**：
  ...
  - 文件命名：`voice-时间戳.txt` 格式
  - 编码格式：UTF-8
  - 存储位置：使用 `process.cwd()` 获取当前工作目录

- **文件生命周期**：
  - 创建：在 TTS 生成前创建临时文件
  - 使用：将 TTS 文本内容写入临时文件
  - 保留：在 TTS 成功时保留临时文件用于调试
  - 清理：通过 `.gitignore` 配置忽略临时文件
...
- **配置与管理**：
  - 目录权限：确保项目目录具有写入权限
  - 磁盘空间：定期清理临时文件，避免磁盘空间不足
  - 版本控制：通过 `.gitignore` 配置忽略临时文件
