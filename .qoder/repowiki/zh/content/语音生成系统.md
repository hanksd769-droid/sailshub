...
## 更新摘要
**变更内容**
- 引入临时文件管理系统：新增 temp 目录和临时文件管理功能，用于 TTS 文件上传机制
- 实现文件上传机制：从直接文本处理改为文件上传方式，支持批处理模式和 SRT 字幕导出
- 增强调试系统：改进了调试记录系统，提供更详细的步骤跟踪和错误诊断能力
- 新增批处理模式：支持批量语音生成和 SRT 字幕导出功能
- 更新 API 接口：新增文件上传和批处理相关接口
...
- TTS 生成流程：支持从文案结果中提取英文行、批量翻译、构建文本并通过文件上传机制调用 Gradio 客户端连接语音生成器进行 TTS（含 MP3 和 SRT 输出），并提供完整的调试与监控。
...
- **更新**：引入临时文件管理系统，从直接文本处理改为文件上传机制，支持批处理模式和 SRT 字幕导出功能。
- **更新**：增强了调试功能，提供更详细的步骤跟踪和错误诊断能力。
...
- [web/src/pages/VoiceGeneratorPage.tsx:1-117](file://web/src/pages/VoiceGeneratorPage.tsx#L1-L117)
...
- 语音路由模块：提供语音配置查询、批量翻译、TTS 生成、调试记录管理等接口，现已集成 Gradio 客户端和临时文件管理。
...
- 临时文件管理系统：新增的临时文件管理功能，支持文件上传和批处理模式。
- 前端语音生成器页面：展示语音服务地址与 API 文档，加载语音生成器 iframe。
- 前端 API 封装：统一处理鉴权头、错误响应与 SSE 流式事件。
- **更新**：Gradio 客户端集成：通过 @gradio/client 连接局域网语音生成器，实现 TTS 功能，支持文件上传机制。
- **更新**：增强的调试记录系统：改进了调试记录存储与管理功能，提供更详细的步骤跟踪和错误诊断能力。
- **更新**：临时文件管理：新增临时文件管理功能，支持文件上传和批处理模式，提高 TTS 生成的灵活性。
...
- **更新**：新增临时文件管理，支持文件上传和批处理模式，提高 TTS 生成的灵活性和效率。
...
**图表来源**
- [api/src/index.ts:1-29](file://api/src/index.ts#L1-L29)
- [api/src/routes/voice.ts:164-206](file://api/src/routes/voice.ts#L164-L206)
- [api/src/routes/voice.ts:210-261](file://api/src/routes/voice.ts#L210-L261)
- [api/src/coze.ts:1-8](file://api/src/coze.ts#L1-L8)
- [web/src/pages/VoiceGeneratorPage.tsx:11-26](file://web/src/pages/VoiceGeneratorPage.tsx#L11-L26)
...
- TTS 生成：将英文行拼接为文本，通过临时文件系统创建 .txt 文件，通过 Gradio 客户端连接局域网语音生成器，依次设置参数并触发生成，返回音频与字幕结果。
- 调试记录：以内存 Map 存储调试记录，限制最大数量；支持按 ID 查询与列表查看。

**更新**：TTS 处理流程已重大改进，引入临时文件管理系统，支持文件上传机制：

**更新功能**：
- **临时文件管理**：新增 runTtsFromTxt 函数，负责创建临时 .txt 文件并进行文件上传。
- **文件上传机制**：通过 File 对象上传 .txt 文件到 Gradio 客户端，支持批处理模式。
- **批处理模式**：启用 lambda 和 lambda_1 参数，支持批量语音生成。
- **SRT 字幕导出**：通过 lambda_4 参数启用 SRT 字幕导出功能。
- **增强的调试记录**：完整的调试记录存储与管理功能，支持步骤跟踪和错误诊断。
...
- **更新**：DebugRecord 类型定义，包含调试记录的完整结构和步骤跟踪功能。
- **更新**：简化的 TTS 处理算法：移除了临时文件管理，直接通过 runTtsFromTxt 函数处理文本生成。
...
BuildTxt --> CreateTemp["创建临时文件<br/>temp/voice-*.txt"]
CreateTemp --> UploadFile["上传文件到 Gradio<br/>File 对象上传"]
UploadFile --> SetParams["设置参数<br/>predict(..., {value})"]
SetParams --> EnableBatch["启用批处理模式<br/>lambda + lambda_1"]
EnableBatch --> EnableSRT["启用SRT导出<br/>lambda_4"]
EnableSRT --> Generate["触发生成<br/>predict('/generate_audio')"]
Generate --> Success["返回成功结果"]
Success --> Done(["结束"])
Err --> Done
```

**图表来源**
- [api/src/routes/voice.ts:89-130](file://api/src/routes/voice.ts#L89-L130)
- [api/src/routes/voice.ts:164-206](file://api/src/routes/voice.ts#L164-L206)
- [api/src/routes/voice.ts:210-261](file://api/src/routes/voice.ts#L210-L261)
...
**章节来源**
- [api/src/routes/voice.ts:11-29](file://api/src/routes/voice.ts#L11-L29)
- [api/src/routes/voice.ts:34-60](file://api/src/routes/voice.ts#L34-L60)
- [api/src/routes/voice.ts:253-270](file://api/src/routes/voice.ts#L253-L270)

### 临时文件管理系统
**更新**：新增的临时文件管理功能，支持文件上传机制：

- **临时文件创建**：在 runTtsFromTxt 函数中创建 temp 目录和临时 .txt 文件。
- **文件上传**：使用 File 对象上传 .txt 文件到 Gradio 客户端，确保文件名为 input.txt。
- **批处理模式**：通过 lambda 和 lambda_1 参数启用批处理功能。
- **SRT 字幕导出**：通过 lambda_4 参数启用 SRT 字幕导出功能。
- **参数配置**：支持多种参数设置，包括增强开关、采样率、步数等。
...
- **更新**：临时文件管理：新增临时文件系统，支持文件上传和批处理模式，提高 TTS 生成的灵活性。
- **更新**：文件上传优化：通过 File 对象上传 .txt 文件，支持批处理模式和 SRT 字幕导出。
- **更新**：批处理模式：启用 lambda 和 lambda_1 参数，支持批量语音生成，提高处理效率。
- **更新**：SRT 字幕导出：通过 lambda_4 参数启用 SRT 字幕导出功能，满足多场景需求。
...
- **更新**：临时文件创建失败
  - 现象：runTtsFromTxt 函数抛出文件创建异常。
  - 处理：检查 temp 目录权限，确认磁盘空间充足，验证文件系统写入权限。
- **更新**：文件上传失败
  - 现象：lambda_2 参数设置时报文件上传错误。
  - 处理：确认文件内容编码正确，检查文件大小限制，验证 Gradio 客户端文件上传功能。
- **更新**：批处理模式异常
  - 现象：lambda 和 lambda_1 参数设置失败。
  - 处理：检查语音生成器是否支持批处理功能，确认参数值范围正确。
- **更新**：SRT 字幕导出失败
  - 现象：lambda_4 参数设置失败或 SRT 文件未生成。
  - 处理：确认语音生成器支持 SRT 导出功能，检查输出目录权限。
- 调试记录缺失
  - 现象：无法通过 /api/voice/debug/:id 查到记录。
  - 处理：确认调试记录未被清理（超过 50 条会删除最早记录）。
- **更新**：临时文件相关问题
  - 现象：系统不再使用临时文件管理。
  - 处理：这是预期行为，系统已移除临时文件管理系统。
- **更新**：API 基础地址配置错误
  - 现象：前端请求指向错误的主机地址。
...
**章节来源**
- [api/src/routes/voice.ts:68-85](file://api/src/routes/voice.ts#L68-L85)
- [api/src/routes/voice.ts:296-322](file://api/src/routes/voice.ts#L296-322)
- [api/src/routes/voice.ts:201-206](file://api/src/routes/voice.ts#L201-L206)
- [api/src/routes/voice.ts:210-251](file://api/src/routes/voice.ts#L210-L251)
- [api/src/routes/voice.ts:253-270](file://api/src/routes/voice.ts#L253-L270)
...
- **更新**：引入临时文件管理系统，支持文件上传机制，提高 TTS 生成的灵活性和效率。
- **更新**：增强的调试功能，提供完整的步骤跟踪和错误诊断能力。
- **更新**：支持批处理模式和 SRT 字幕导出功能，满足多样化需求。
- **更新**：简化的 @gradio/client 集成，提高了系统的稳定性和性能。
...
**章节来源**
- [api/src/routes/voice.ts:68-85](file://api/src/routes/voice.ts#L68-L85)
- [api/src/routes/voice.ts:273-338](file://api/src/routes/voice.ts#L273-L338)
- [api/src/routes/voice.ts:340-399](file://api/src/routes/voice.ts#L340-L399)
- [api/src/routes/voice.ts:253-270](file://api/src/routes/voice.ts#L253-L270)
...
- **更新**：Gradio 客户端配置：确保局域网语音生成器支持 Gradio 协议，验证客户端版本兼容性。
- **更新**：调试记录管理：系统自动管理最多 50 条调试记录，超出数量会自动清理最早的记录。
- **更新**：临时文件管理：新增临时文件管理系统，支持文件上传和批处理模式，提高 TTS 生成的灵活性。
...
- **调试与监控**：
  ...
  3. 分析调试记录中的步骤日志和错误信息
  4. **更新**：查看步骤跟踪信息，了解每个操作的具体参数和结果
  5. **更新**：监控临时文件创建和文件上传过程
...
- **功能特性**：
  - 自动创建临时文件：runTtsFromTxt 函数负责创建临时 .txt 文件
  - 文件上传支持：通过 File 对象上传 .txt 文件到 Gradio 客户端
  - 批处理模式：支持批量语音生成和 SRT 字幕导出
  - 参数配置：支持多种参数设置，包括增强开关、采样率、步数等
